...
function HabitsHeatmapSlide() {
  ...
  const intensities = Array.from({ length: cols * rows }, () => Math.random());
  return (
    <div className="select-none">
...
function TimerSessionsSlide() {
  const sessions = [
    { label: "Session 1", dur: "25:00", done: true  },
    { label: "Break",     dur: "05:00", done: true, isBreak: true },
    { label: "Session 2", dur: "15:32", done: false },
...
// ── Forced dependency chains ───────────────────────────────────────────────────
// Expansion is transitive: daily-focus → projects → progress (two hops).
// Deselecting a parent cascades cleanup of its forced deps unless user independently picked them.

const FORCED_DEPS: Record<string, string[]> = {
  "time-tracker": ["activity-log"],
  "activity-log": ["time-tracker"], // inverse: selecting activity-log forces focus-timer
  "daily-focus":  ["projects"],     // projects then forces progress transitively
  "projects":     ["progress"],
  "progress":     ["projects"],     // inverse: selecting progress forces projects
};

// ── Contextual suggestion copy ─────────────────────────────────────────────────
...
export default function ActiveWidgetsModal({ isOpen, onClose, initialSelected, onSave }: Props) {
  ...
  return (
    <div className="fixed inset-0 z-[60] flex items-center justify-center p-4">
      ...
      <div className="relative w-full max-w-lg bg-[#0B0F19] border border-white/[0.09] rounded-2xl shadow-2xl flex flex-col max-h-[92vh] overflow-hidden">
        ...
        <div className="px-5 py-4 border-t border-white/[0.06] flex gap-3 shrink-0">
          ...
          <button
            ...
            className="flex-1 h-10 rounded-xl text-sm font-semibold text-white disabled:opacity-40 transition-all shadow-[0_0_20px_rgba(139,92,246,0.35)]"
